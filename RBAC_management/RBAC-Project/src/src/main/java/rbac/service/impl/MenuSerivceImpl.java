...
public class MenuSerivceImpl extends ServiceImpl<MenuMapper, Menu> implements IMenuService {
    ...
    public List<Menu> getTreelist() {
        ...
        for(Menu menu : menuList){
            if(menu.getPid() != -1l)
                continue;
//            Collections.sort(menu.getChildren(), Comparator.comparingInt(Menu::getOrderNum));
            treelist.add(menu);
        }
        return treelist;
    }
    ...
    @Override
    public Menu getInfoByidWithpidName(Long id) {
        return baseMapper.getInfoByidWithpidName(id);
    }
